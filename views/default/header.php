<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge"/>
    <meta name="viewport" content="width=device-width, initial-scale=1">

    <meta name="author" content="Enrique Corona Ricaño">
    <meta name="description" content="Sistema en PHP para tienda en linea por entregable de actividad">
    <meta name="keywords" content="Enrique Corona Ricaño, Maestria Unir, Dirección e ingenieria de sitios web">

    <!-- Bootstrap -->
    <link rel="stylesheet" href="<?=base_url()?>assets/framework/bootstrap45/css/bootstrap.min.css">

    <!-- iconos fontawesome -->
    <link href="<?=base_url()?>assets/framework/fontawesome/css/all.css" rel="stylesheet">

    <!-- CSS para el plugin de las notificaciones del sistema -->
    <link href="<?=base_url()?>assets/framework/watnotif/css/bubble/watnotif.right-bottom-bubble.css" rel="stylesheet" type="text/css">

    <!-- css para el fileupload -->
    <link href="<?=base_url()?>assets/framework/fileupload/css/jquery.fileupload.css" rel="stylesheet">

    <title>Tienda UNIR v2</title>
    <!-- scritps para el pago -->

    <link rel="icon" href="<?=base_url()?>assets/img/logo_ecr.png">

    <link href="<?=base_url()?>assets/css/master.css" rel="stylesheet">
</head>

<body>

<div id="backgroundImage" class="fullscreen-bg"></div>